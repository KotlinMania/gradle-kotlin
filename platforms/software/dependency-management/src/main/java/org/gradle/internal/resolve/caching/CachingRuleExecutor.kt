 ...
 * [ instantiating action][<], which is responsible for instantiating
 * and configuring a rule. From a user point of view, a rule works on a [DETAILS] instance,
 * which is often, but not necessarily, a builder. A [DETAILS] can mutate an existing object,
 ...
 * the result of executing the rule from the [RESULT] object.
 *
...
interface CachingRuleExecutor<KEY, DETAILS, RESULT> {
    /**
     ...
     */
    fun <D : DETAILS?> execute(
        key: KEY?,
        rule: InstantiatingAction<DETAILS?>?,
