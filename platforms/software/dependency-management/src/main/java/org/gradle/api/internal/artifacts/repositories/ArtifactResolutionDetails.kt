 ...
 *
 *  * when looking for a specific module version, for example org:foo:1.0
 *  * when looking for the list of versions for a module, for example org:foo
 *
 *
 *
 ...
 * then it's enough to just look at the module id using [.getModuleId].
 *
 *
 ...
 * then you must look at the version thanks to the [.getComponentId] method. But
 * because there can also be version listings, you must also check for [.getModuleId].
 *
 *
 * A [convenience method][.isVersionListing] makes it easier to find out if you
 * are in the version listing case, or module version case.
 *
 *
 * Filtering is done by calling the [.notFound] method: as soon as you know a module
