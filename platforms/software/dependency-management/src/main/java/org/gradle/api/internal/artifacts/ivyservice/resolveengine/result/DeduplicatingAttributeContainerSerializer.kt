...
/**
 * A serializer for [AttributeContainer] that deduplicates the values and delegates to
 ...
 */
class DeduplicatingAttributeContainerSerializer(private val delegate: AttributeContainerSerializer) : AttributeContainerSerializer {
    ...
    private val writeIndex = IdentityHashMap<AttributeContainer, Int>()
    private val readIndex: MutableList<ImmutableAttributes> = ArrayList<ImmutableAttributes>()

    @Throws(IOException::class)
    override fun read(decoder: Decoder): ImmutableAttributes {
        val empty = decoder.readBoolean()
        if (empty) {
    ...
    override fun write(encoder: Encoder, container: AttributeContainer) {
        ...
        val idx = writeIndex.get(container)
        if (idx == null) {
            // new value
