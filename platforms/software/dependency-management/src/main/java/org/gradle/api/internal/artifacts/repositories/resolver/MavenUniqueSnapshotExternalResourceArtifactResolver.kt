...
internal class MavenUniqueSnapshotExternalResourceArtifactResolver(private val delegate: ExternalResourceArtifactResolver, private val snapshot: MavenUniqueSnapshotModuleSource) :
    ...
    protected fun timestamp(artifact: ModuleComponentArtifactMetadata): ModuleComponentArtifactMetadata {
        val snapshotComponentIdentifier =
            MavenUniqueSnapshotComponentIdentifier(artifact.getId()!!.getComponentIdentifier(), snapshot.getTimestamp())
        return DefaultModuleComponentArtifactMetadata(snapshotComponentIdentifier, artifact.getName())
    }
}
