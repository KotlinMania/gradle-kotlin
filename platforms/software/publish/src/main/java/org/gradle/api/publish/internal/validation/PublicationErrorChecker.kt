 ...
 */
abstract class PublicationErrorChecker {
    companion object {
        /**
         ...
         */
        fun checkForUnpublishableAttributes(component: SoftwareComponentInternal, documentationRegistry: DocumentationRegistry) {
            for (variant in component.getUsages()) {
                variant.getAttributes().keySet().stream()
                    .filter { attribute: Attribute<*>? -> Category.CATEGORY_ATTRIBUTE.getName() == attribute!!.getName() }
                    .findFirst()
                    .ifPresent(Consumer { attribute: Attribute<*>? ->
                        val value: Any? = variant.getAttributes().getAttribute(attribute)
                        if (value != null && Category.VERIFICATION == value.toString()) {
                            throw PublishException(
                                "Cannot publish module metadata for component '" + component.getName() + "' which would include a variant '" + variant.getName() + "' that contains a '" +
