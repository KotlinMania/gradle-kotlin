...
class InjectedAutoAppliedPluginRegistry(private val buildDefinition: BuildDefinition) : AutoAppliedPluginRegistry {
    ...
    override fun getAutoAppliedPlugins(target: Settings): PluginRequests {
        if ((target.getStartParameter() as StartParameterInternal).isUseEmptySettings) {
            return PluginRequests.EMPTY
        }
