...
import org.gradle.internal.deprecation.DeprecationLogger.deprecate
...
class BuildResultOutputFeatureCheck(private val targetGradleVersion: GradleVersion, private val embedded: Boolean) : FeatureCheck {
    override fun verify() {
    ...
    companion object {
        private val LOGGER: Logger = Logger.getLogger(BuildResultOutputFeatureCheck::class.java.getName())

        fun warnIfUnsupportedVersion(targetGradleVersion: GradleVersion, messageSupplier: Function<String?, String?>) {
            if (targetGradleVersion.compareTo(DefaultGradleConnector.MINIMUM_SUPPORTED_GRADLE_VERSION) < 0) {
                LOGGER.warning(
                    (messageSupplier.apply(targetGradleVersion.getVersion())
            ...
            @Suppress("unused") val keepTheDeprecationLoggerCall = Runnable {
                deprecate("Running TestKit with unsupported old Gradle versions.")
