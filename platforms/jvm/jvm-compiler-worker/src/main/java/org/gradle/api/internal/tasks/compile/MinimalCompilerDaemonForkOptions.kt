 ...
 * worker executor.  Since [ProviderAwareCompilerDaemonForkOptions]
 * and its subclasses can accept user-defined [org.gradle.process.CommandLineArgumentProvider]
 * instances, these objects may contain references to mutable objects in the
 * Gradle model or other non-isolatable objects.
 *
 * Subclasses should be sure to collapse any [org.gradle.process.CommandLineArgumentProvider]
 * arguments into [.jvmArgs] in order to capture the user-provided
 * command line arguments.
 */
open class MinimalCompilerDaemonForkOptions(forkOptions: BaseForkOptions) : BaseForkOptions() {
    init {
