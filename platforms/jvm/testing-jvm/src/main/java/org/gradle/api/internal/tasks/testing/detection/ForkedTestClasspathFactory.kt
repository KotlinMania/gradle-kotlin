 ...
 */
class ForkedTestClasspathFactory(private val moduleRegistry: ModuleRegistry) {
    fun create(
        classpath: Iterable<out File?>,
        modulepath: Iterable<out File?>
    ): ForkedTestClasspath {
        return ForkedTestClasspath(
            ImmutableList.copyOf<File?>(classpath),
            ImmutableList.copyOf<File?>(modulepath),
            withImplementation(ImmutableList.of<URL?>())
        )
    }
     ...
     */
    private fun withImplementation(additionalImplementationClasspath: MutableList<URL?>?): ImmutableList<URL?> {
        return ImmutableList.copyOf<URL?>(
            CollectionUtils.flattenCollections<URL?>(
                URL::class.java,
