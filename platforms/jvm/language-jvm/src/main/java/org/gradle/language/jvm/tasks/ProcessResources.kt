...
abstract class ProcessResources : Copy() {
    override fun copy() {
        val cleanedOutputs = StaleOutputCleaner.cleanOutputs(this.deleter, getOutputs().getPreviousOutputFiles(), getDestinationDir())
        super.copy()
        if (cleanedOutputs) {
            setDidWork(true)
        }
