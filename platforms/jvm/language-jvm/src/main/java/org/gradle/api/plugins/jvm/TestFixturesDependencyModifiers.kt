...
interface TestFixturesDependencyModifiers {
    @get:Nested
    val testFixtures: TestFixturesDependencyModifier?

    /**
     * Implementation for the test fixtures dependency modifier.
     *
     * @since 8.0
     * @see .modifyImplementation
     */
    @Incubating
    class TestFixturesDependencyModifier : DependencyModifier() {
        /**
         * {@inheritDoc}
         *
         * Selects the test fixtures variant of the given dependency.
         */
        public override fun modifyImplementation(dependency: ModuleDependency) {
            dependency.capabilities(Action { c: ModuleDependencyCapabilitiesHandler? -> c!!.requireFeature(TestFixturesSupport.TEST_FIXTURES_CAPABILITY_FEATURE_NAME) })
        }
    }
}
