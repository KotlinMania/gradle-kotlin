 ...
 * types specific to Java and other JVM languages, such as [HasCompileOptions] which are
 * not available in the `platform-jvm` project and would otherwise be located on the
 * [JvmPluginServices] type.
 */
@ServiceScope(Scope.Project::class)
interface JvmLanguageUtilities {
    /**
     ...
     */
    fun <COMPILE> useDefaultTargetPlatformInference(configuration: Configuration?, compileTask: TaskProvider<COMPILE?>?) where COMPILE : AbstractCompile?, COMPILE : HasCompileOptions?
     ...
     */
    fun registerJvmLanguageSourceDirectory(sourceSet: SourceSet?, name: String?, configuration: Action<in JvmLanguageSourceDirectoryBuilder?>?)
}
