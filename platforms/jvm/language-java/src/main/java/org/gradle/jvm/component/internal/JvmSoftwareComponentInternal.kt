...
/**
 * A [SoftwareComponent] which produces variants intended for use within the JVM ecosystem.
 *
 *
 ...
 * is the lack of a proper variant API and support for automatically aggregating variants of owned features.
 *
 *
 * TODO: Before publicizing this component, we also need to consider how component extensibility works.
 * For example, for the java-library plugin we have the additional `api` and `compileOnlyApi`
 ...
 * Java inheritance?
 *
 * Even though the single implementation of this interface also implements [org.gradle.api.component.AdhocComponentWithVariants],
 * we do not want to extend that interface here, as it is slated for removal.
 */
interface JvmSoftwareComponentInternal : SoftwareComponent {
     ...
     */
    @JvmField
    val features: NamedDomainObjectContainer<JvmFeatureInternal?>?
     ...
     * suites should be accessed via [.getFeatures].
     */
    @JvmField
    val testSuites: NamedDomainObjectContainer<TestSuite?>?

    /**
     * Get the feature which encapsulates all logic and domain objects for building the production software product.
     *
     *
     * This is a legacy API. Plugins should support components with multiple features by calling [.getFeatures].
     */
    @JvmField
