...
class DefaultJvmToolchainsConfigurationValidator(private val startParameter: StartParameterInternal) : JvmToolchainsConfigurationValidator {
    /**
     ...
     * shipping them to the daemon as project properties in [StartParameter].  So, whether a property is set as
     ...
     */
    override fun validatePropertyConfiguration(propertyName: String) {
        val projectProperties: MutableMap<String, String>? = startParameter.projectPropertiesUntracked
        if (projectProperties!!.containsKey(propertyName)) {
            val projectProperty: String = projectProperties.get(propertyName)!!
            val systemProperties = System.getProperties()
            if (systemProperties.containsKey(propertyName)) {
                val systemProperty = systemProperties.get(propertyName)
                if (projectProperty != systemProperty) {
     ...
     */
    override fun validateAllPropertiesConfigurationsForDaemonJvmToolchains() {
    ...
    companion object {
        private fun emitDeprecatedWarning(propertyName: String, value: String) {
            deprecateAction("Specifying '" + propertyName + "' as a project property on the command line")
