 ...
 */
class AggregatingProcessor(delegate: Processor?, result: AnnotationProcessorResult?) : DelegatingProcessor(delegate) {
    private val strategy: IncrementalProcessingStrategy

    init {
        this.strategy = AggregatingProcessingStrategy(result)
    }
