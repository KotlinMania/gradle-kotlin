...
internal class GccLinker(
...
) : AbstractCompiler<LinkerSpec?>(buildOperationExecutor, commandLineToolInvocationWorker, invocationContext, GccLinkerArgsTransformer(), useCommandFile, workerLeaseService) {
    ...
    private class GccLinkerArgsTransformer : ArgsTransformer<LinkerSpec?> {
        ...
        fun maybeSetInstallName(spec: SharedLibraryLinkerSpec, args: MutableList<String?>) {
            val installName = spec.getInstallName()
            val targetOs = spec.getTargetPlatform().operatingSystem
