...
interface GccCompatibleToolChain : NativeToolChain {
     ...
     *
     * @param pathEntries The path values to append. These are evaluated as per [org.gradle.api.Project.files]
     */
     ...
     */
    fun target(platformName: String?, action: Action<in GccPlatformToolChain?>?)
     ...
     */
    fun eachPlatform(action: Action<in GccPlatformToolChain?>?)
}
