...
interface FileMetadata {
     ...
     * How the file with the metadata was accessed.
     *
     * [AccessType] only gives information about the queried path, not about parents of the queried path.
     *
     * If we have a symlink situation like `symlink1` -&gt; `symlink2` -&gt; `target`,
     * then the metadata for `symlink1` will have an access type [AccessType.VIA_SYMLINK]
     ...
     * metadata of `symlinkedDir/fileInDir` will be [AccessType.DIRECT],
    ...
    enum class AccessType {
        ...
        companion object {
             ...
             */
            @JvmStatic
            fun viaSymlink(viaSymlink: Boolean): AccessType {
                return if (viaSymlink) AccessType.VIA_SYMLINK else AccessType.DIRECT
            }
        }
    }
}
