...
object ZipEntryConstants {
     ...
     * local time zone (as MS-DOS date/time). Use [.MINIMUM_TIME_FOR_ZIP_ENTRIES_UTC] when comparing against a raw (UTC) timestamp instead.
     */
    @JvmField
    val CONSTANT_TIME_FOR_ZIP_ENTRIES: Long = GregorianCalendar(1980, Calendar.FEBRUARY, 1, 0, 0, 0).getTimeInMillis()
     ...
     * This is the UTC-instant form of [.CONSTANT_TIME_FOR_ZIP_ENTRIES]: once adjusted to the default
     * time zone for storage it produces the same 1980-02-01 entry date, independent of the build time zone.
     */
    @JvmField
    val MINIMUM_TIME_FOR_ZIP_ENTRIES_UTC: Long = Instant.parse("1980-02-01T00:00:00Z").toEpochMilli()
     ...
     */
    @JvmField
    val MAXIMUM_TIME_FOR_ZIP_ENTRIES_UTC: Long = Instant.parse("2097-11-01T00:00:00Z").toEpochMilli()
}
