...
object ArgumentsSplitter {
     ...
     */
    @JvmStatic
    fun split(arguments: String): MutableList<String> {
        val commandLineArguments: MutableList<String> = ArrayList<String>()

        var currentQuote: Char? = null
        val currentOption = StringBuilder()
        var hasOption = false

        for (index in 0..<arguments.length) {
            val c = arguments.get(index)
