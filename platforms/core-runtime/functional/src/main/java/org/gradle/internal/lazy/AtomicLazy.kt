 ...
 * @see Lazy.atomic
 */
internal class AtomicLazy<T : Any?>(supplier: Supplier<T?>) : Lazy<T?> {
    // The type argument is as nullable as T, but it cannot be expressed naturally in Java.
    private val value = AtomicReference<Any?>(UNINITIALIZED)
    ...
    override fun get(): T? {
        ...
        return Cast.unsafeStripNullable<T?>(Cast.uncheckedCast<T?>(value.get()))
    }
