...
internal class DefaultClassFileTransformer : ClassFileTransformer {
    override fun transform(
        ...
        if (loader !is InstrumentingClassLoader) {
            return null
        }
        val instrumentingLoader = loader as InstrumentingClassLoader
        try {
    ...
    companion object {
        ...
        fun tryInstall(): Boolean {
            ...
            check(INSTALLED.compareAndSet(false, true)) { "The transformer is already installed in " + DefaultClassFileTransformer::class.java.getClassLoader() }
            return AgentControl.installTransformer(DefaultClassFileTransformer())
        }
    }
}
