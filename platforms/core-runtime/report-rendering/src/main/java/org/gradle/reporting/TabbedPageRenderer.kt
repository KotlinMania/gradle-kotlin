...
abstract class TabbedPageRenderer<T> : ReportRenderer<T?, HtmlPageBuilder<SimpleHtmlWriter?>?>() {
    ...
    override fun render(model: T?, builder: HtmlPageBuilder<SimpleHtmlWriter>) {
        ...
        val baseStyleLink = builder.requireResource(BASE_STYLE_URL)
        val reportJsLink = builder.requireResource(REPORT_JS_URL)
        val styleLink = builder.requireResource(this.styleUrl)

        val htmlWriter = builder.getOutput()
        ...
        htmlWriter.startElement("div").attribute("id", "footer")
            ...
            .characters(String.format(" at %s", builder.formatDate(Date())))
            .endElement()
            .endElement()
            .endElement()
            .endElement()
