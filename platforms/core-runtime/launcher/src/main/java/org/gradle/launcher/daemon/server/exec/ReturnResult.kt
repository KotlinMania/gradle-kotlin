 ...
 */
class ReturnResult : DaemonCommandAction {
    override fun execute(execution: DaemonCommandExecution) {
        execution.proceed()

        val result: Result<*> = Success(execution.result)

        LOGGER.debug("Daemon is dispatching the build result: {}", result)
        execution.connection!!.completed(result)
    }

    companion object {
        private val LOGGER: Logger = Logging.getLogger(ReturnResult::class.java)
    }
}
