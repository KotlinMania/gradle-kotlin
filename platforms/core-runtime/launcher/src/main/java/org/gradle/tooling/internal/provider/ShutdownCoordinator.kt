...
class ShutdownCoordinator(private val client: DaemonStopClientExecuter) : DaemonStartListener {
    private val daemons: MutableMap<File, MutableSet<DaemonConnectDetails>> = HashMap<File, MutableSet<DaemonConnectDetails>>()
    private val incorrectDaemonRegistryPath: File

    init {
        this.incorrectDaemonRegistryPath = DaemonParameters(BuildLayoutConverter().defaultValues().getGradleUserHomeDir(), null).getBaseDir()
    }

    override fun daemonStarted(daemonDir: DaemonDir, daemon: DaemonConnectDetails) {
        synchronized(daemons) {
            val startedDaemons: MutableSet<DaemonConnectDetails>
            if (!daemons.containsKey(daemonDir.getBaseDir())) {
                startedDaemons = HashSet<DaemonConnectDetails>()
    ...
    fun stopStartedDaemons(requestSpecificLoggingServices: ServiceRegistry, daemonBaseDir: File) {
        synchronized(daemons) {
            val startedDaemons = daemons.get(daemonBaseDir)
            if (startedDaemons != null && !startedDaemons.isEmpty()) {
                try {
                    client.execute(requestSpecificLoggingServices, daemonBaseDir, Consumer { daemonStopClient: DaemonStopClient? -> daemonStopClient!!.gracefulStop(startedDaemons) })
                } finally {
    ...
    fun stopAllDaemons() {
        ...
        synchronized(daemons) {
            ...
            for (startedDaemons in daemons.values) {
                try {
                    client.execute(requestSpecificLoggingServices, incorrectDaemonRegistryPath, Consumer { daemonStopClient: DaemonStopClient? -> daemonStopClient!!.gracefulStop(startedDaemons) })
                } finally {
                    startedDaemons.clear()
                }
            }
