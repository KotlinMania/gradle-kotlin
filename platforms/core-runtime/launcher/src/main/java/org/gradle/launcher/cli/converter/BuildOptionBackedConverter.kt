...
class BuildOptionBackedConverter<T>(private val buildOptions: BuildOptionSet<T?>) {
    fun configure(parser: CommandLineParser?) {
        buildOptions.commandLineConverter().configure(parser)
    }

    fun convert(commandLine: ParsedCommandLine?, properties: MutableMap<String?, String?>, environment: MutableMap<String?, String?>, target: T?) {
