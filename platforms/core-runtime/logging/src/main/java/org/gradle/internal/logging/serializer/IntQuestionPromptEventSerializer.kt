...
class IntQuestionPromptEventSerializer : Serializer<IntQuestionPromptEvent> {
    ...
    override fun write(encoder: Encoder, value: IntQuestionPromptEvent) {
        ...
        encoder.writeSmallInt(value.minValue)
        encoder.writeSmallInt(value.defaultValue)
    }

    @Throws(Exception::class)
    override fun read(decoder: Decoder): IntQuestionPromptEvent {
        return IntQuestionPromptEvent(decoder.readLong(), decoder.readString(), decoder.readSmallInt(), decoder.readSmallInt())
    }
}
