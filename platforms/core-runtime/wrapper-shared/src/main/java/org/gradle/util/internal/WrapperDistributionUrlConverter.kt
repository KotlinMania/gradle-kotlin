 ...
 */
object WrapperDistributionUrlConverter {
    /**
    ...
    fun convertDistributionUrl(distributionUrl: String, fileRoot: File?): URI {
        val source = URI(distributionUrl)
        if (source.getScheme() == null) {
            //  No scheme means someone passed a relative url.
            //  In our context only file relative urls make sense.
            return File(fileRoot, source.getSchemeSpecificPart()).toURI()
        } else {
            return source
        }
    }
