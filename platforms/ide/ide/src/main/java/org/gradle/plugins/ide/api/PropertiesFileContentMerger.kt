 ...
 */
class PropertiesFileContentMerger(var transformer: PropertiesTransformer) : FileContentMerger() {
    /**
     * Adds a closure to be called when the file has been created.
     * The [Properties] are passed to the closure as a parameter.
     ...
     */
    fun withProperties(closure: Closure<*>?) {
        withProperties(ClosureBackedAction<Properties?>(closure, Closure.OWNER_FIRST))
    }

    /**
     * Adds an action to be called when the file has been created.
     * The [Properties] are passed to the action as a parameter.
     ...
     */
    fun withProperties(action: Action<Properties?>) {
