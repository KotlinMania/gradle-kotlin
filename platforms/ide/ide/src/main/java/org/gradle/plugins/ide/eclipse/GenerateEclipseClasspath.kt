...
abstract class GenerateEclipseClasspath : XmlGeneratorTask<Classpath?> {
    /**
     * The Eclipse Classpath model containing the information required to generate the classpath file.
     */
    @get:Internal
    var classpath: EclipseClasspath? = null
    ...
    @Inject
    constructor(classpath: EclipseClasspath?) {
    ...
    override fun create(): Classpath {
        return Classpath(getXmlTransformer(), classpath!!.getFileReferenceFactory())
    }

    override fun configure(xmlClasspath: Classpath?) {
