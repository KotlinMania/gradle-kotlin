 ...
 */
@NullMarked
internal object IdeaIsolatedProjectsWorkarounds {
    /**
     ...
     */
    fun hasPlugin(project: Project, pluginClass: Class<out Plugin<*>>): Boolean {
        return (project as ProjectInternal).getOwner().fromMutableState<Boolean>(Function { p: ProjectInternal? -> p!!.getPluginManager().getPluginContainer().hasPlugin(pluginClass) }
        )
    }
}
