...
abstract class PropertyListGeneratorTask<T : PersistableConfigurationObject?> : GeneratorTask<T?>() {
    @get:Internal
    val propertyListTransformer: PropertyListTransformer<*> = PropertyListTransformer<Any?>()

    init {
        generator = object : PersistableConfigurationObjectGenerator<T?>() {
            override fun create(): T? {
                return this@PropertyListGeneratorTask.create()
            }

            override fun configure(`object`: T?) {
                this@PropertyListGeneratorTask.configure(`object`)
            }
        }
    }
