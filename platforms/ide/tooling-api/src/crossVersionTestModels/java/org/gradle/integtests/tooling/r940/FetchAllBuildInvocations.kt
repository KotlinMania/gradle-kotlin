...
internal class FetchAllBuildInvocations : BuildAction<AllBuildInvocationsResult?>, Serializable {
    public override fun execute(controller: BuildController): AllBuildInvocationsResult {
        ...
        val rootInvocations: BuildInvocations? = controller.getModel(BuildInvocations::class.java)

        // Get BuildInvocations for each included build
        val includedInvocations: MutableList<BuildInvocations?> = ArrayList<BuildInvocations?>()
        for (includedBuild in gradleBuild.getIncludedBuilds()) {
            includedInvocations.add(controller.getModel(includedBuild, BuildInvocations::class.java))
        }

        return AllBuildInvocationsResult(rootInvocations, includedInvocations)
    }
}
