...
class Action1 : BuildAction<MutableList<String?>?> {
    public override fun execute(controller: BuildController?): MutableList<String?> {
        return mutableListOf<String?>(controller.getBuildModel().getRootProject().getName() + " " + (++SharedActionStaticState.counter))
    }
}
