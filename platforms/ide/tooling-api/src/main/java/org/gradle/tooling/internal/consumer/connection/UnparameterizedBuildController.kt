...
internal abstract class UnparameterizedBuildController(
    ...
    override fun getBuildModel(): GradleBuild {
        return getModel<GradleBuild>(null, GradleBuild::class.java)!!
    }
    ...
    protected abstract fun getModel(target: Any?, modelIdentifier: ModelIdentifier, parameter: Any?): BuildResult<*>?
    ...
    override fun <T> run(actions: MutableCollection<out BuildAction<out T>>): MutableList<T?> {
        val results: MutableList<T?> = ArrayList<T?>(actions.size)
    ...
    override fun send(value: Any) {
        throw UnsupportedVersionException(String.format("Gradle version %s does not support streaming values to the client.", gradleVersion.getVersion()))
    }
    ...
    override fun <M, P> fetch(target: Model?, modelType: Class<M?>, parameterType: Class<P?>?, parameterInitializer: Action<in P?>?): FetchModelResult<M?> {
        try {
            val model: Any = getModel<M?, P?>(target!!, modelType, parameterType!!, parameterInitializer!!)!!
            return DefaultFetchModelResult.success<M?>(uncheckedNonnullCast<M?>(model))
        } catch (e: Exception) {
    ...
    companion object {
        protected fun <P> initializeParameter(parameterType: Class<P?>?, parameterInitializer: Action<in P?>): P? {
