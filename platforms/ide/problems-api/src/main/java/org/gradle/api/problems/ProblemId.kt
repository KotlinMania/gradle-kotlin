 ...
 * From the name fields a fully qualified name can be inferred: `compilation:java:unused-variable`.
...
protected constructor() {
    ...
    companion object {
         ...
         *
         * @param name the name of the problem. The convention is to use kebab-case (ie lower case with hyphens). Cannot be blank (i.e., `null`, empty string, or only whitespaces).
         * @param displayName the user-friendly display name of the problem. Cannot be blank (i.e., `null`, empty string, or only whitespaces).
         * @param group the group to which the problem belongs. Cannot be null.
         * @return the new problem id
         * @since 8.13
         */
        @JvmStatic
        fun create(name: String, displayName: String, group: ProblemGroup): ProblemId {
            return DefaultProblemId(name, displayName, group)
        }
    }
}
