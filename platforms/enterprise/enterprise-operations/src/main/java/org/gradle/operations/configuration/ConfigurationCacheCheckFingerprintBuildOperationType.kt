/*
 * Copyright 2024 the original author or authors.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.gradle.operations.configuration

import org.gradle.internal.operations.BuildOperationType

/**
 * Details about the configuration cache fingerprint check operation.
 *
 * @since 8.10
 */
class ConfigurationCacheCheckFingerprintBuildOperationType :
    BuildOperationType<ConfigurationCacheCheckFingerprintBuildOperationType.Details?, ConfigurationCacheCheckFingerprintBuildOperationType.Result?> {
    /**
     * Build operation details.
     *
     * @since 8.10
     */
    interface Details

    /**
     * Result of the fingerprint check. The fingerprint can be:
     *
     *  * **Not found**: there is no cached configuration for the key. Corresponds to [CheckStatus.NOT_FOUND].
     * Both [.getBuildInvalidationReasons] and [.getProjectInvalidationReasons] are empty.
     *  * **Valid**: the cached configuration can be fully reused. Corresponds to [CheckStatus.VALID].
     * Both [.getBuildInvalidationReasons] and [.getProjectInvalidationReasons] are empty.
     *
     *  * **Partially invalidated**: some projects are invalidated and have to be reconfigured.
     * The configuration is rebuilt but cached data of the unaffected projects is reused.
     * Corresponds to [CheckStatus.PARTIAL].
     * The [.getBuildInvalidationReasons] list is empty.
     * The [.getProjectInvalidationReasons] list contains information about invalidated projects.
     *
     *  * **Fully invalidated**: some of the build-level inputs has changed. The entry is discarded and configuration phase reruns from scratch.
     * Corresponds to [CheckStatus.INVALID].
     * The [.getBuildInvalidationReasons] list contains the invalidation reasons.
     * The [.getProjectInvalidationReasons] list is empty.
     *
     *
     * @since 8.10
     */
    interface Result {
        /**
         * Returns the overall status of the fingerprint check.
         *
         * @return the status
         * @since 8.10
         */
        val status: CheckStatus?

        /**
         * Returns the list of the build fingerprint invalidation reasons. Can be empty if all build-level inputs are up-to-date.
         * The entry may still be partially invalidated if any of the project-scoped fingerprints are invalid.
         *
         *
         * Not all invalidation reasons may be returned. However, if the entry is invalidated, at least one reason is listed.
         *
         *
         * The first invalidation of the first build in the list is what Gradle shows to the user.
         *
         * @return the list of invalidation reasons
         * @since 8.10
         */
        val buildInvalidationReasons: MutableList<BuildInvalidationReasons>?

        /**
         * Returns a list of project-level invalidation reasons. Only contains entries for projects that were invalidated.
         * This list is empty if the whole build is invalidated, i.e. when [.getBuildInvalidationReasons] returns a non-empty list.
         *
         *
         * All invalidated project have entries in this list, however, not all invalidation reasons may be listed.
         *
         *
         * The first invalidation of the first project in the list is what Gradle shows to the user.
         * The rest of the invalidated projects follows, ordered by the [ProjectInvalidationReasons.getBuildPath], then [ProjectInvalidationReasons.getProjectPath].
         *
         * @return a list of invalidation projects with their invalidation reasons
         * @since 8.10
         */
        val projectInvalidationReasons: MutableList<ProjectInvalidationReasons>?

        /**
         * The ID of the build that produced the configuration cache entry we compared against.
         *
         * `null` when unknown, e.g. when there is no previous fingerprint.
         *
         * @since 8.13
         */
        val originBuildInvocationId: String?
    }

    /**
     * Contains invalidation for a single build.
     *
     * @since 8.10
     */
    interface BuildInvalidationReasons {
        /**
         * Returns the path of the invalidated build in the build tree.
         *
         * @return the build path
         * @since 8.10
         */
        val buildPath: String?

        /**
         * Returns the list of the invalidation reasons for the given build. At least one invalidation is always present.
         *
         * @return the list of invalidation reasons
         * @since 8.10
         */
        val invalidationReasons: MutableList<FingerprintInvalidationReason>?
    }

    /**
     * Contains data of a single invalidated project.
     *
     * @since 8.10
     */
    interface ProjectInvalidationReasons {
        /**
         * Returns the build path of the invalidated project.
         *
         * @return the build path
         * @since 8.10
         */
        val buildPath: String?

        /**
         * Returns the project path of the invalidated project within the build.
         *
         * @return the project path
         * @since 8.10
         */
        val projectPath: String?

        /**
         * Returns the list of invalidation reasons. At least one invalidation is always present.
         *
         * @return the list of invalidation reasons
         * @since 8.10
         */
        val invalidationReasons: MutableList<FingerprintInvalidationReason>?
    }

    /**
     * Represents the invalidation reason.
     *
     * @since 8.10
     */
    interface FingerprintInvalidationReason {
        /**
         * Returns a simple text representation of the invalidation reason.
         *
         * @return the invalidation reason as text
         * @since 8.10
         */
        val message: String?
    }

    /**
     * An overall result of the fingerprint check.
     *
     * @since 8.10
     */
    enum class CheckStatus {
        /**
         * Fingerprint file was not found. Most likely, there is no cached configuration for the given key. No invalidation reasons are reported.
         *
         * @since 8.10
         */
        NOT_FOUND,

        /**
         * The cached entry is valid and can be reused. No invalidation reasons are reported.
         *
         * @since 8.10
         */
        VALID,

        /**
         * Configuration of some projects is invalidated. Configuration phase is going to be re-executed but the data from unaffected project will be reused.
         * A list of invalidated projects and their invalidation reasons can be found in [Result.getProjectInvalidationReasons].
         *
         * @since 8.10
         */
        PARTIAL,

        /**
         * The cached entry is invalid. Configuration phase is going to be re-executed from scratch. A list of invalidation reasons can be found in [Result.getBuildInvalidationReasons].
         *
         * @since 8.10
         */
        INVALID
    }
}
