...
interface DevelocityPluginUnsafeConfigurationService {
    /**
     * Run some code without tracking its inputs.
     *
     *
     * All code that runs during `supplier.get()` is exempt from configuration input tracking.
     ...
     */
    fun <T> withConfigurationInputTrackingDisabled(supplier: Supplier<T?>): T?
}
