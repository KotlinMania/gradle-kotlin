...
interface GradleEnterprisePluginBackgroundJobExecutors {
    /**
     * Returns the executor to run user-provided background jobs in the background workers.
     * The intended use is to provide workers for the `buildScan.background` callbacks.
     * The jobs will be complete before [GradleEnterprisePluginEndOfBuildListener.buildFinished] is called.
     *
     *
     * The job may be rejected if the build is already finishing and the executor is being shut down.
     * In such a case a [RejectedExecutionException] is thrown from [Executor.execute].
     *
     *
     * The job should not throw exceptions.
     * Any exceptions thrown are going to fail the build and may prevent the `buildFinished` callback from firing.
     *
     *
     ...
     */
    val userJobExecutor: Executor?

    /**
     * Returns `true` if the current thread is running the background job submitted to one of executors owned by this class.
     */
